...
int readBytes(ifstream& file, int byteSize) {
    ...
    for (int i = 0; i < byteSize; i++) {
        x = (x << 8) | bytes[i];  // Shift accumulated value and add next byte
    }
    return x;
}
...
vector<vector<float>> loadMNISTImages(ifstream& file) {
    ...
    int numImages = readBytes(file, 4);
    int numRows = readBytes(file, 4);
    int numCols = readBytes(file, 4);
    ...
    for (int i = 0; i < numImages; i++) {
        ...
        for (int j = 0; j < numRows * numCols; j++) {
            image.push_back(readBytes(file,1) / 255.0f);
        }
