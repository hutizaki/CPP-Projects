...
TrainedWeights runTrainingGPU(const vector<vector<float>> &images, const vector<int> labels, 
    ...
    vector<vector<float>> w1 = ml::generateRandMatrix(numNeurons, inputLength);
    vector<vector<float>> w2 = ml::generateRandMatrix(10, numNeurons);
    vector<float> b1 = ml::generateRandVector(numNeurons);
    vector<float> b2 = ml::generateRandVector(10);
